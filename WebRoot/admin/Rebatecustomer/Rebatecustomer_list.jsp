...
<html>
	...
	$(function(){
		$('#dg').datagrid({    
				//请求的url地址
			    url:'Rebatecustomer/Rebatecustomer-listByCustId.action', 
			    queryParams :{custId:''},
			   loadMsg:'请等待...',
				//隔行换色——斑马线
				fit:true,
				...
				toolbar: [{
					...
					handler: function(){
						$("#win").window({
							...
							width:'650',
							height:'80%',
							content:'<iframe src="Rebatecustomer_saveInput.jsp" frameborder="0" width="100%" height="100%"> </iframe>'
						});
					...
					},
					{
						...
						if(rows.length !=1){
							...
							$("#win").window({
								...
								width:'1025',
								height:'645',
								content:'<iframe title="" src="Rebatecustomer_updateInput.jsp" frameborder="0" width="100%" height="100%"/>'
							});
					}}},
					{
						iconCls: 'icon-remove',
						...
						if(rows.length ==0){
							...
							$.messager.confirm("删除确认对话框","是否要删除选中的记录",function(r){
								if(r){
									...
									//发送ajax请求
									$.post("Rebatecustomer-deleteByIds.action",{ids:ids},function(result){
										if(result =="true"){
											$.messager.show({
												title:'删除成功',
												msg:'删除成功',
			    ...
			    columns:[[ 			      
					        ...
					        {field:'name',title:'收货人名',align:'center'},
