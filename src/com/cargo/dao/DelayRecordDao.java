...
@Component
public class DelayRecordDao extends BaseDao {
	public void save(DelayRecord transientInstance) {
	...
	public void delete(DelayRecord persistentInstance) {
	...
	public void deleteByIds(String ids) {
		//log.debug("deleting Rebate instance");
		try {
			String hql ="delete from DelayRecord where id in ("+ ids +")";
			getSession().createQuery(hql).executeUpdate();
		} catch (RuntimeException re) {
			throw re;
		}
	}
	public void update(DelayRecord sender){
		try {
	...
	public DelayRecord findById(java.lang.Integer id) {
		//log.debug("getting Rebate instance with id: " + id);
		try {
			DelayRecord instance = (DelayRecord) getSession().get("com.cargo.model.DelayRecord", id);
			return instance;
		} catch (RuntimeException re) {
			throw re;
		}
	...
	public List findAll() {
		...
		try {
			String queryString = "from DelayRecord";
	...
	public void approvalDelayRecord(String ids,Integer status){
			try {
				String hql ="update DelayRecord w set w.approval=:approval  where id in ("+ ids +")";			
	...
	public Map find(String custId,String bitch,String waybill,String sender,String rater,String lineId,Date stdate,Date enddate){
		...
		Criteria crit = getSession().createCriteria(DelayRecord.class);
		...
		List<DelayRecord> comps = (List<DelayRecord>)crit.list();
	...
	public Long countFeeByWaybill(String waybill){
		...
		try {
			String queryString = "select sum(fee) from DelayRecord i where i.waybill =:waybill";
	...
	public Boolean isExsit(String waybill,Integer reason){
		boolean isExsit = false;
		List list = null;
			
			try {
				String queryString = "from DelayRecord i where i.waybill =:waybill and i.reason =:reason";
				list = getSession().createQuery(queryString)  
				.setString("waybill",waybill)
				.setInteger("reason",reason)
				.list();
